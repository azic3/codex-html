...
- 将数据库连接信息迁移到环境变量或配置文件
- 为用户表增加邮箱字段，并校验手机号与邮箱绑定关系
- 为验证码、登录和上传接口增加限流与失败次数限制
- 将密码哈希升级为 Argon2id 或 bcrypt
- 为上传文件增加 MIME 校验、随机存储名和清理策略
- 增加核心接口的 curl 验证脚本或自动化测试
- 增加日志、HTTPS/反向代理和 systemd 部署示例
