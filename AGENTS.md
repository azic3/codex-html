...
- 将 MySQL 账号、数据库名、端口等启动配置从 `main.cpp` 迁移到环境变量或配置文件。
- 给用户表增加邮箱字段，并在注册和找回密码时校验手机号与邮箱绑定关系。
- 为邮箱验证码增加发送频率限制、失败重试、验证码尝试次数限制和过期清理策略。
- 将 SHA-512 crypt 密码哈希升级为 Argon2id 或 bcrypt，并提供清晰的哈希版本迁移方案。
- 为上传文件增加更严格的 MIME 校验、随机存储名、访问权限控制和过期清理机制。
- 为登录、注册、验证码、图片上传、视频上传等核心接口补充最小 curl 验证脚本或自动化测试。
- 增加运行日志与错误日志，并对密码、验证码、SMTP 授权码等敏感信息做脱敏。
- 增加 HTTPS/反向代理部署示例，例如 Nginx + systemd 服务配置。
- 为前端补充更明确的错误提示、上传进度显示和大文件上传失败恢复体验。
- 增加数据库初始化 SQL 或迁移脚本，避免手动创建/修改表结构。
